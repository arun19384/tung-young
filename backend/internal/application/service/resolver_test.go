...
func TestAsokToSiamAndReverse(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		got, err := r.Resolve(ResolveRequest{l.ID, tc.dest, samplesAt(a, b, []float64{.35, .4, .45}, now)}, now)
		if err != nil {
			t.Fatal(err)
		}
...
func TestRejectUnreliableFixes(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			got, err := r.Resolve(ResolveRequest{l.ID, "CEN", tc.edit(samplesAt(a, b, []float64{.3, .35, .4}, now))}, now)
...
func TestArrivalRequiresRepeatedFixes(t *testing.T) {
	...
	got, err := r.Resolve(ResolveRequest{l.ID, "CEN", s}, now)
...
func TestInvalidDestinationAndTimestamp(t *testing.T) {
	...
	if _, err := r.Resolve(ResolveRequest{l.ID, "BL01", s}, now); err == nil {
		t.Fatal("must reject wrong-line destination")
	}
	s[2].Timestamp = s[1].Timestamp
	if _, err := r.Resolve(ResolveRequest{l.ID, "CEN", s}, now); err == nil {
