  ...
  return (
    <div className="route-map">
      <div className="route-map-heading">
        ...
        <button type="button" onClick={() => setExpanded(true)}><ZoomIn size={14} /> ขยายแผนที่</button>
      </div>
      {drawing}
      ...
      {expanded && (
        <div className="route-map-modal" role="dialog" aria-modal="true" aria-label="แผนที่การเดินทางแบบขยาย">
          <div className="route-map-modal-header">
            <strong>แผนที่การเดินทาง</strong>
            <button type="button" aria-label="ปิดแผนที่" onClick={() => setExpanded(false)}><X /></button>
          </div>
          <div className="route-map-viewport">
            <div style={{ width: `${zoom * 100}%` }}>{drawing}</div>
          </div>
          <div className="route-map-controls">
            <button type="button" aria-label="ซูมออก" disabled={zoom <= 1} onClick={() => setZoom((value) => Math.max(1, value - .5))}><Minus /></button>
            <span>{Math.round(zoom * 100)}%</span>
            <button type="button" aria-label="ซูมเข้า" disabled={zoom >= 3} onClick={() => setZoom((value) => Math.min(3, value + .5))}><Plus /></button>
          </div>
        </div>
      )}
    </div>
